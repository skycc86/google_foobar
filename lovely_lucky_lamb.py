...
def answer(total_lambs):
    ...
    # if total_lambs <10 or total_lambs > 10**9:
      # print "ERROR - invalid total_lambs !!!"
      # raise ValueError
      # return
    max_men = min_men = 0
    max_num = total_lambs - 1 - 2
    current_lamb = 2
    while max_num >= (current_lamb * 2):
      next_lamb = current_lamb * 2
      max_num -= next_lamb
      current_lamb = next_lamb
      max_men += 1
      print 'max ', next_lamb
    min_num = total_lambs - 1 - 1
    current_lamb = previous_lamb = 1
    next_lamb = 0
    while min_num >= (previous_lamb + current_lamb):
      next_lamb = current_lamb + previous_lamb
      min_num -= next_lamb
      previous_lamb = current_lamb
      current_lamb = next_lamb
      min_men += 1
      print 'min ', next_lamb
    return min_men - max_men
